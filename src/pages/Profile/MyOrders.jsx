import { useEffect, useState } from 'react'

import ProfileLayout from './ProfileLayout'
import { getOrders } from '../../api/orders'

function MyOrders() {
  const [rows, setRows] = useState([])
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState('')

  useEffect(() => {
    let alive = true

    const run = async () => {
      try {
        setIsLoading(true)
        setError('')
        const res = await getOrders()
        const list = Array.isArray(res?.data) ? res.data : []
        if (!alive) return
        setRows(list)
      } catch (e) {
        if (!alive) return
        setError(e?.message || 'Failed to load orders')
      } finally {
        if (!alive) return
        setIsLoading(false)
      }
    }

    run()
    return () => {
      alive = false
    }
  }, [])

  return (
    <ProfileLayout activeId="orders" crumb="My Order" title="My Order">
      <div className="overflow-hidden rounded-lg border border-[#F3F4F6]">
        <div className="grid grid-cols-[140px_1fr_120px_120px_120px_120px] bg-[#FAFAFA] px-6 py-3 text-[12px] font-medium text-[#6B7280]">
          <div>Order id</div>
          <div>Products</div>
          <div>Date</div>
          <div>Amount</div>
          <div>Details</div>
          <div>Review</div>
        </div>
        {isLoading && (
          <div className="px-6 py-6 text-[12px] text-[#6B7280]">Loading…</div>
        )}

        {!isLoading && error && <div className="px-6 py-6 text-[12px] text-[#EC2323]">{error}</div>}

        {!isLoading && !error && rows.map((r, idx) => (
          <div
            key={`${r.id || idx}-${idx}`}
            className="grid grid-cols-[140px_1fr_120px_120px_120px_120px] items-center px-6 py-5 text-[12px]"
          >
            <div className="text-[#9CA3AF]">{r.id ?? '-'}</div>
            <div className="flex items-center gap-4">
              <button type="button" className="text-[#9CA3AF]">‹</button>
              <div className="h-14 w-14 rounded-md bg-[#E5E7EB]" />
              <button type="button" className="text-[#9CA3AF]">›</button>
            </div>
            <div className="text-[#9CA3AF]">{r.created_at || r.date || '-'}</div>
            <div>{r.total || r.amount || '-'}</div>
            <button type="button" className="text-[#EC2323]">View details</button>
            <button type="button" className="text-[#EC2323]">Review order</button>
          </div>
        ))}
      </div>

      <div className="mt-6 flex justify-center gap-2">
        <button type="button" className="h-7 w-7 rounded-sm bg-[#EC2323] text-[12px] text-white">
          1
        </button>
        <button type="button" className="h-7 w-7 rounded-sm border border-[#E5E7EB] text-[12px] text-[#6B7280]">
          2
        </button>
        <button type="button" className="h-7 w-7 rounded-sm border border-[#E5E7EB] text-[12px] text-[#6B7280]">
          3
        </button>
      </div>
    </ProfileLayout>
  )
}

export default MyOrders
